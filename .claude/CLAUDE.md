...
`seedTemplateSampleRows_` などの初期サンプルデータは、`createRequirementsSheet` 実行のたびに
全シートが上書きされる前提のもの（フォーマットを一度示せば目的は達成される）。「網羅性のため」
という理由だけで似たようなサンプル行・冗長な例を追加しないこと。追加する前に、それが本当に
新しい情報を伝えているか（既存のサンプル・ガイドの説明で既にカバーされていないか）を確認する。
指摘や改善提案を鵜呑みにせず、メンテコストに見合う価値があるかどうかを都度判断してよい。
